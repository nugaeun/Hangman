...
class Hangman():
    ...
    def __init__(self, word_to_guess):
        ...
        self.game_progress = list() # 알파벳 중복 확인용
    ...
    def find_indexes(self, letter):
        ...
        """
        return [char[i] for i, char in enumerate(self.word_to_guess) if letter in self.word_to_guess]
    ...
    def is_invalid_letter(self, input_):
        ...
        """
        return (input_ == int) or (input_.isalpha() and len(input_) >= 1)
    ...
    def update_progress(self, letter, indexes):
        ...
        for letter in self.word_to_guess:
            self.game_progress.append(letter)
    ...
    def play(self):
        ...
        """
        while True:
            print('게임을 실행합니다. ') # 현재 게임 상태를 먼저 출력해줌. 
            user_input = Hangman.get_user_input(self) # user_input 을 입력받는 메소드 적용. 

            # Validate the user input
            if not Hangman.is_invalid_letter(user_input, str(user_input)): # 플레이어가 입력한 인풋이 유효한 값인지 아닌지. 
            ...
            if user_input in self.game_progress:
                print('이미 입력했다. ') # 이미 입력받았던 알파벳이면, 이미 입력했다 메시지 출력
